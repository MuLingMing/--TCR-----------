...
在这种情况下，请使用 SampleData 文件夹中的数据作为输入示例。
该文件夹包含 4 个文件，均由 Adaptive Biotechnology 制作，可从 immuneAccess 下载。
&nbsp; &nbsp; &nbsp;&nbsp;
...
如果从 AdaptiveBiotech immuneAnalyzer 生成的原始 TCR 重排测序数据开始，请在 DeepCAT 目录中创建一个文件夹（your_folder_name），并将输入的 "tsv "文件放在其中。DeepCAT 目录包含原始输入文件 Example_raw_file.tsv。
&nbsp; &nbsp; &nbsp;&nbsp;
...
要从头开始训练 DeepCAT，请使用 TrainingData 文件夹中的示例数据。该文件夹包含两个文件，每个文件都是来自癌症或健康个体的 CDR3 列表。
在终端运行以下命令：
```python
python
>>> from DeepCAT import *
